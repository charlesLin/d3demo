...
svg.selectAll("circle").data(data)
  ...
  .on('mousemove', function (d) {
    ...
    console.log(d3.select(this), cx, cy);
    //var {clientX, clientY} = d3.event;
    var shiftX = cx + 20;
    hintGroup.classed("hidden", false)
      .attr('style', `top: ${cy}px; left: ${cx}px;`);
...
var maxRps = Math.max.apply(Math, data.map(d => d.rps));
var maxElment = data.find(x => x.rps == maxRps);
var maxElementIndex = data.indexOf(maxElment);

console.log("maxRps", maxRps, maxElment, maxElementIndex);
console.log(data, data[maxElementIndex]);

for (let i = 0; i < data.length; i++) {
  var e = d3.select(`circle:nth-child(${i})`);
}
d3.select(`circle:nth-child(${maxElementIndex})`).dispatch('mousemove');